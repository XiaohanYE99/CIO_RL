...
class MixedTanhGaussianPolicy(Mlp, TorchStochasticPolicy):
    """
    Usage:
    ...
    def __init__(
        ...
        super().__init__(
            hidden_sizes,
            input_size=obs_dim,
            output_size=action_dim+contact_dim*(2-1),
            init_w=init_w,
            **kwargs
        )
        self.log_std = None
        self.std = std
        self.action_dim=action_dim-contact_dim
        self.contact_dim=contact_dim
        if std is None:
            last_hidden_size = obs_dim
            if len(hidden_sizes) > 0:
                last_hidden_size = hidden_sizes[-1]
            self.last_fc_log_std = nn.Linear(last_hidden_size, self.action_dim)
            self.last_fc_log_std.weight.data.uniform_(-init_w, init_w)
            self.last_fc_log_std.bias.data.uniform_(-init_w, init_w)
        else:
            self.log_std = np.log(std)
    ...
    def forward(self, obs):
        ...
        prob=output[:,self.action_dim:].view(-1,self.contact_dim,2)
        ...
        if self.std is None:
            ...
            std = torch.from_numpy(np.array([self.std, ])).float().to(
                ptu.device)
        return Categorical(prob),TanhNormal(mean, std)

    def logprob(self, contact,action, prob,mean, std):
        tanh_normal = TanhNormal(mean, std)
        log_prob = tanh_normal.log_prob(
            action,
        )
        log_prob = log_prob.sum(dim=1, keepdim=True)
        cate_prob=Categorical(prob)
        log_prob1=cate_prob.log_prob(contact)
        log_prob+=log_prob1.sum(dim=1, keepdim=True)
        return log_prob
...
class CategoricalPolicy(Mlp, TorchStochasticPolicy):
    def __init__(
        ...
        super().__init__(
            hidden_sizes,
            input_size=obs_dim,
            output_size=action_dim[0]*action_dim[1],
            init_w=init_w,
            output_activation=torch.tanh,
            **kwargs
        )
    ...
    def forward(self, obs):
        ...
        for i, fc in enumerate(self.fcs):
            h = self.hidden_activation(fc(h))
        preactivation = self.last_fc(h)
        prob = self.output_activation(preactivation).view(-1,self.action_dim[0],self.action_dim[1])
        prob = torch.softmax(prob,dim=2)
